...
`docker run -d --cap-add NET_ADMIN -p 500:500/udp -p 4500:4500/udp -p 1701:1701/tcp -p 1194:1194/udp -p 5555:5555/tcp vishnupradeep/softethervpn`
...
To obtain a config file template, `docker run` the initial setup with Server & Hub passwords, then `docker cp` out the config file:
    ...
    $ docker run --name vpnconf -e SPW=<serverpw> -e HPW=<hubpw> vishnupradeep/softethervpn echo
    $ docker cp vpnconf:/usr/vpnserver/vpn_server.config /path/to/vpn_server.config
    $ docker rm vpnconf
    $ docker run ... -v /path/to/vpn_server.config:/usr/vpnserver/vpn_server.config vishnupradeep/softethervpn
...
`docker run -d --cap-add NET_ADMIN -p 1194:1194/udp vishnupradeep/softethervpn`
...
`docker run --rm vishnupradeep/softethervpn gencert > /path/to/envlist`
...
[MIT License][4].
  ...
  [4]: https://github.com/vishnupradeep/softethervpn/raw/master/LICENSE
